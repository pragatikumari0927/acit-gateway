...
2. C4 Prompt Firewall receives raw content, refuses bidi/zero-width code points as IDPI, rejects poison keys and substrings, recurses structures. Those code points are also stripped before substring matching so they cannot split a denylisted phrase; `\u00ad` (soft hyphen) is the one exception — normalised away rather than refused, because it occurs in real Catalog product text.
...
See `ARCHITECTURE.md` for Mermaid diagrams of flow, components, boundaries, and data models.
...
- `Proposal`: mandate_id, merchant_id (Catalog lookup key; not a DB column), items, quoted_total_paise, quoted_discount_paise, copy (strings scanned by Guardrails).
- `CatalogItem`: sku, name, description, unit_amount_paise, inventory, discount_bounds (`min_percent`/`max_percent`), categories.
